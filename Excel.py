...
def verify_admin_pin():
    # Look for pin query param or JSON body pin
...
def admin_users():
    if not verify_admin_pin():
        return jsonify({'error': 'Unauthorized: Invalid PIN'}), 403
    users = User.query.all()
    users_list = []
    for u in users:
        users_list.append({
...
# --- HTML Template includes new PIN modal and updated scripts ---
TEMPLATE_HTML = """
<!DOCTYPE html>
<html lang="en">
<head>
...
<!-- PIN Modal -->
<div id="pin-modal" class="fixed inset-0 hidden items-center justify-center z-70">
  <div class="bg-white rounded-lg p-8 w-80 max-w-full shadow-lg flex flex-col items-center">
    <h3 class="text-2xl font-bold mb-6 text-gray-900 text-center">Enter Admin PIN</h3>
    <input id="pin-input" type="password" maxlength="6" autocomplete="off" inputmode="numeric"
           class="text-center text-2xl font-mono border border-gray-300 rounded-md p-3 w-full mb-4 focus:outline-yellow-400 focus:ring-2 focus:ring-yellow-400" 
           placeholder="6-digit PIN" />
    <p id="pin-error" class="text-red-600 font-semibold mb-4 text-center hidden"></p>
    <div class="flex space-x-4 w-full">
      <button id="pin-cancel-btn" class="bg-gray-400 hover:bg-gray-500 text-white font-bold py-2 px-6 rounded-md shadow-md transition flex-grow">Cancel</button>
      <button id="pin-submit-btn" class="bg-yellow-400 hover:bg-yellow-500 text-black font-bold py-2 px-6 rounded-md shadow-md transition flex-grow" disabled>Submit</button>
    </div>
  </div>
</div>
...
<script>
  ...
  // We'll intercept Admin and Export button clicks to request PIN first
  ...
  document.getElementById('export-excel-btn').addEventListener('click', async () => {
    ...
    if (!pin) return;
    window.location.href = '/export?pin=' + encodeURIComponent(pin);
  });
  ...
  document.getElementById('mobile-export-excel-btn').addEventListener('click', async () => {
    const pin = await requestPin();
    if (!pin) return;
    window.location.href = '/export?pin=' + encodeURIComponent(pin);
    mobileMenu.classList.add('hidden');
  ...
  // Admin tools buttons & user list handling
  ...
  // To store admin PIN in session (JS memory only)
  let adminPin = null;
  ...
  pinInput.addEventListener('input', () => {
    ...
    pinSubmitBtn.disabled = (val.length !== 6 || !/^[0-9]{6}$/.test(val));
    pinError.classList.add('hidden');
  });
  ...
  async function requestPin() {
    return new Promise(resolve => {
      ...
      function cleanup() {
        pinSubmitBtn.removeEventListener('click', handleSubmit);
        pinCancelBtn.removeEventListener('click', handleCancel);
        pinInput.removeEventListener('keydown', onEnter);
      }

      pinSubmitBtn.addEventListener('click', handleSubmit);
      pinCancelBtn.addEventListener('click', handleCancel);
      pinInput.addEventListener('keydown', onEnter);
    })
  }

  // Wrapper to request PIN once and re-use for subsequent admin actions
  async function requestPinAndThen(callback) {
    if (adminPin) {
      callback();
      return;
    }
    ...
    // Test PIN via a test API call (e.g. fetching users)
    try {
      const res = await fetch('/admin/users?pin=' + encodeURIComponent(pin));
      if(res.status === 403) {
        pinError.textContent = "Unauthorized: Invalid PIN";
        pinError.classList.remove('hidden');
        adminPin = null;
        await requestPinAndThen(callback);  // Recurse to prompt again
        return;
      }
      if(!res.ok) throw new Error('Error verifying PIN');
      adminPin = pin;
      callback();
    } catch(e){
  ...
  adminSearchUserBtn.addEventListener('click', async () => {
    if (!adminPin) {
      await requestPinAndThen(() => {});  // First ensure PIN is entered
      if(!adminPin) return;
    }
    ...
    try {
      const res = await fetch('/admin/user/' + regNo + '?pin=' + encodeURIComponent(adminPin));
      if(res.status === 403){
        adminMessage.textContent = 'Unauthorized: Invalid PIN';
        adminMessage.className = 'mt-6 text-center font-semibold text-red-400';
        adminPin = null;
        return;
      }
  ...
  adminViewUsersBtn.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch('/admin/users?pin=' + encodeURIComponent(adminPin));
      if(res.status === 403) {
        adminMessage.textContent = 'Unauthorized: Invalid PIN';
        adminMessage.className = 'mt-6 text-center font-semibold text-red-400';
        adminPin = null;
  ...
  adminDeleteUserBtn.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch('/admin/user/' + regNo + '?pin=' + encodeURIComponent(adminPin), { method: 'DELETE' });
      if(res.status === 403){
        adminMessage.textContent = 'Unauthorized: Invalid PIN';
        adminMessage.className = 'mt-6 text-center font-semibold text-red-400';
        adminPin = null;
